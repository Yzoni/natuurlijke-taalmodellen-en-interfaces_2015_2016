...
def no_smoothing(ngram_count, n_1_gram_count, test_file, sequence_size, voc_size):
...
if __name__ == "__main__":
    ...
    if args.smoothing == 'no':
        ...
        probabilities = no_smoothing(ngram_count, n_1_gram_count,
                                     args.test_corpus, args.n,voc_size)
        c = Counter(probabilities)
        print("percentage of sentences assigned zero probability:",
              c[0]/len(probabilities))
        pprint(probabilities)
    elif args.smoothing == 'add1':
        voc_size = get_vocabulary_size(args.training_corpus)
        probabilities = add_one_smoothing(ngram_count, n_1_gram_count,
                                          args.test_corpus, args.n,voc_size)
        c  = Counter(probabilities)
        print("percentage of sentences assigned zero probability:",
              c[0]/len(probabilities))
        ...
        probabilities = good_turing_smoothing(ngram_count, args.test_corpus,
                                              args.n, length_ngrams)
        c  = Counter(probabilities)
        print("percentage of sentences assigned zero probability:",
              c[0]/len(probabilities))
